...
export default function Editor({ script, setScript, files, matchedTokens, currentIndex }) {
    ...
    return (
        <div className="flex-1 flex flex-col min-h-0 bg-[#141414] rounded-xl border border-[#1F1F1F] overflow-hidden shadow-xl">
            ...
            <div className="flex-1 relative overflow-hidden">
                <div
                    ref={highlightRef}
                    className="absolute inset-0 p-8 whitespace-pre-wrap break-words font-['Malgun_Gothic'] text-[22px] leading-[1.8] pointer-events-none tracking-tight"
                >
                    {matchedTokens.map((t, i) => (
                        <span
                            key={i}
                            className={`
                                ${t.source === 'script' ? 'text-[#B4A7D6]' : t.source === 'video' ? 'text-[#A2C4C9]' : t.source === 'audio' ? 'text-[#A4C2F4]' : 'text-[#444]'}
            ...
            <div className="p-4 bg-[#0F0F0F] border-t border-[#1F1F1F] overflow-x-auto">
                <div className="flex gap-2 min-w-max pb-2">
                    {recommendations.length === 0 && (
                        <span className="text-[10px] text-[#444] italic">추천 파일이 여기에 표시됩니다.</span>
                    )}
                    {recommendations.map((rec, i) => (
                        <button
                            key={i}
                            onClick={() => applyRecommendation(rec.name)}
                            className="px-3 py-1.5 rounded-lg bg-[#1A1A1A] border border-[#262626] text-[11px] font-bold hover:border-white text-zinc-300 hover:text-white transition-all flex items-center gap-2"
                        >
